...
public class Quiz02Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        if (intent != null) {
            quiz = (Quiz02) intent.getSerializableExtra("Quiz02");
            show();
        }
    }
    ...
    public void answer(View view){
        ...
        Log.d("one_count",four_count+"カウント");
        if(four_count == max){
            type = "Dタイプ";
        ...
        Log.d("two_count",two_count+"カウント");
        if(two_count == max){
            type = "Bタイプ";
            Intent intent = new Intent(getApplication(),QuizResult.class); // 結果の画面へ移動
            intent.putExtra("Type",type);
        ...
        quiz = Quiz02.getQuiz02(quiz.q_num + 1);
        if (quiz != null) {
            show();
        } else {
            Intent intent = new Intent(getApplication(),QuizResult.class); // 結果の画面へ移動
